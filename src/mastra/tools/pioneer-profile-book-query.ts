...
export default createTool({
  ...
  outputSchema: z.object({
    ...
    metadata: z
      .object({
        queryType: z
          .enum(['aggregate', 'specific_field', 'matching', 'general', 'all'])
          .optional()
        ...
        filterField: z
          .string()
          .optional()
          .describe('Field filter applied (skills, industries, roles, etc.)'),
      })
      .optional()
    ...
    // Handle matching queries - find pioneers by skills, roles, industries
      ...
      // Search by industries
        ...
        for (const pioneer of allPioneers) {
          ...
          if (
            searchTerms.some((term) => industriesStr.includes(term)) ||
